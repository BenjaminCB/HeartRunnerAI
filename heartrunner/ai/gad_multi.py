...
STATE_mip = numpy.zeros(RUNNER_COUNT)
PREV_TIME = 0
SAMPLE_CHANCE = 0.03
...
def update_state_time_mip(t: hct.MultiTask):
    global PREV_TIME, STATE_mip

    STATE_mip -= (t.time - PREV_TIME)
    STATE_mip = STATE_mip.clip(min=0)
    PREV_TIME = t.time
...
def greedy_mip(mt: hct.MultiTask):
    ...
    # insert 10.000 in 'amount of col' for BLANK in 'amount of row'
...
if __name__ == "__main__":
    ...
    for task in tasks:
        ...
        if gre_mip > gre or gre_mip < gre:
            print('task:', i)
            print('Greedy: ', gre, 'Greedy_mip: ', gre_mip)
            print(task)
            print(STATE)
            print(STATE_mip)
        i += 1
